...
if __name__ == '__main__':
    ...
    print "Task: K selection for factor analysis"
    ...
    print "Write the number of K to try, separated by commas"
    k_list1 =  [int(n) for n in raw_input().split(",")]
    ...
    name_res =  "mod1_resk_%s.txt" %"_".join([str(s) for s in k_list1])
    results_1 = pd.DataFrame(results_1, columns = ["K", "BestModel", "AUC_test", "AUC_train", "AUC_CV", "Acc_test", "Acc_train", "params"]) 
    results_1.to_csv(name_res, index=None, sep=' ', mode='a')
